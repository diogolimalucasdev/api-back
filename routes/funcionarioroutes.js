...
router.post("/cadastrar", async (req, res) => {
  ...
  if (nomeCompleto.length < 2)
    return res
      .status(400)
      .json({ error: "Insira nome e sobrenome do funcionário!" });
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Funcionario inserido com sucesso", funcionario }); //dado criado com sucesso
  } catch (error) {
...
router.patch("/atualizar/:id", async (req, res) => {
  ...
  try {
    const atualizarFuncionario = await Funcionario.updateOne(
      { _id: id },
      funcionario
    );

    if (atualizarFuncionario.matchedCount === 0) {
      //valido se o usuario existe ou nao
      res.status(400).json({ message: "funcionario nao encontrado" });
      return;
    }
